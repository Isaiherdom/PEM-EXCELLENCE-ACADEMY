# PEM Excellence Academy — Kit de Módulo 11
### ISO 14001 · Cl. 9.1.2 · Cumplimiento Legal Ambiental · Nivel 4 Experto · Escuela Ambiental

---

## 1. Prompt completo para Gamma (copiar y pegar)

```
Crea una presentación ejecutiva de máximo 8 diapositivas titulada "ISO 14001 — Módulo 11: Cumplimiento Legal Ambiental" para PEM Excellence Academy, Escuela Ambiental de Production Enhancement México (PEM).

Mantén exactamente la misma identidad visual de los módulos anteriores: fondo oscuro casi negro, tipografía geométrica de alto contraste, acento azul único (#4C9AFF), estilo minimalista ejecutivo, máximo 25 palabras por diapositiva.

Estructura (una idea por diapositiva):
1. Hook — "El permiso seguía 'vigente' en el tablero. Nadie había verificado si realmente se cumplía."
2. Registro vs. evaluación — el Módulo 3 identifica qué aplica, este módulo verifica si se cumple.
3. Qué exige la Cl. 9.1.2 — frecuencia, método, y conocimiento continuo del estado de cumplimiento.
4. Ejemplo real: la bitácora de evaluación de cumplimiento legal de los sitios de trabajo.
5. Autoevaluación antes de la inspección — detectar y corregir por cuenta propia.
6. Un incumplimiento detectado se trata como no conformidad — análisis de causa raíz.
7. Riesgo de frecuencias de evaluación inconsistentes entre sitios de trabajo.
8. Cierre — invitación a tomar el examen del Módulo 11 y obtener el certificado digital.

Usa imágenes de una revisión de cumplimiento en campo, un documento legal con sello de verificación, y un calendario de evaluaciones programadas. Incluye "PEM Excellence Academy — Escuela Ambiental — Módulo 11" en la portada.
```

---

## 2. Storyboard de las 8 diapositivas

| # | Diapositiva | Elemento visual dominante | Texto en pantalla |
|---|---|---|---|
| 1 | Hook | Sello "VIGENTE" con signo de interrogación | "¿Pero se cumple de verdad?" |
| 2 | Registro vs. evaluación | Dos iconos: lista vs. lupa | "Saber qué aplica no es suficiente" |
| 3 | Qué exige Cl. 9.1.2 | Icono de calendario + verificación | "Frecuencia, método, conocimiento continuo" |
| 4 | Ejemplo real | Bitácora de evaluación (igual a la infografía) | "Evaluación activa, no solo fecha" |
| 5 | Autoevaluación | PEM revisándose antes que la autoridad | "Detectar antes que ellos" |
| 6 | Tratamiento de incumplimiento | Icono de causa raíz | "No conformidad, no sorpresa" |
| 7 | Riesgo | Dos calendarios con frecuencias distintas | "Sin criterio, expuestos por más tiempo" |
| 8 | Cierre | CTA grande | "Toma el examen → obtén tu certificado" |

---

## 3. Imágenes recomendadas

- Revisión de cumplimiento ambiental en campo, documento y realidad comparados
- Documento legal con sello o marca de verificación
- Calendario o cronograma de evaluaciones programadas por sitio de trabajo
- Icono lineal de lupa sobre un documento
- Persona de HSE completando una bitácora de evaluación

Evitar: imágenes de trámite gubernamental genérico sin relación con verificación operativa real.

---

## 4. Guion completo para avatar (3:30–4:30 min)

**[0:00–0:20 — Hook]**
"El permiso de un sitio de trabajo seguía marcado como 'vigente' en el tablero que vimos en el Módulo 3. Pero nadie había verificado en meses si las condiciones reales de operación cumplían lo que ese permiso exigía. Tener el registro no es lo mismo que saber si se cumple."

**[0:20–1:00 — Registro vs. evaluación]**
"El Módulo 3 nos dio el registro de requisitos legales: qué obligaciones aplican a cada sitio de trabajo. Este módulo va un paso más allá — la cláusula 9.1.2 exige verificar activamente si esas obligaciones realmente se están cumpliendo en la operación diaria, no solo tenerlas archivadas."

**[1:00–1:40 — Qué exige Cl. 9.1.2]**
"PEM debe determinar con qué frecuencia evalúa su cumplimiento legal, con qué método, y mantener un conocimiento continuo de su estado — no solo saberlo cuando llega una auditoría."

**[1:40–2:20 — Ejemplo real]**
"Así se ve en la práctica: una bitácora que registra, por cada requisito legal, cuándo fue la última evaluación, el resultado, y cuándo será la próxima. A diferencia del tablero del Módulo 3, que solo mostraba fechas de vigencia, esta bitácora muestra si la condición real de operación cumple lo que exige el permiso."

**[2:20–2:55 — Autoevaluación]**
"El valor real de esto es hacerlo antes de que la autoridad lo haga por nosotros. Una autoevaluación interna permite detectar y corregir un incumplimiento por cuenta propia, antes de que se convierta en un hallazgo externo o una sanción."

**[2:55–3:30 — Tratamiento del incumplimiento]**
"Y cuando la evaluación detecta un incumplimiento —como el límite de emisiones en un sitio de trabajo—, se trata exactamente igual que cualquier otra no conformidad del sistema: se analiza la causa raíz y se toman acciones correctivas, no solo se documenta y se olvida."

**[3:30–4:05 — Riesgo y cierre]**
"Si dos sitios de trabajo evalúan su cumplimiento legal con frecuencias muy distintas, sin un criterio definido, el sitio de trabajo con menor frecuencia queda expuesta por más tiempo a un incumplimiento no detectado. Con esto terminas el Módulo 11. Toma el examen de 10 preguntas — necesitas 80% para aprobar — y genera tu certificado digital. Nos vemos en el Módulo 12, el último de esta escuela."

*(Duración estimada: 4:10–4:30 min)*

---

## 5. Prompt para HeyGen (o Synthesia)

```
Avatar: el mismo personaje de los módulos anteriores de la Escuela Ambiental, para continuidad de identidad.
Fondo: estudio neutro oscuro/gris grafito con acento azul (#4C9AFF), idéntico a módulos anteriores.
Voz: español mexicano neutro, mismo ritmo pausado y directo.
Guion: usar el texto de la sección 4 de este documento, dividido en los mismos 6 bloques.
Duración objetivo: 4:10–4:30 minutos.
Gestos: énfasis en el bloque de la bitácora (gesto de recorrer una lista) y en el cierre.
Subtítulos: activados, español, sincronizados.
Salida: 1080p 16:9 para SharePoint/Teams + recorte 9:16 para celular.
```

---

## 6. Indicadores de desempeño del módulo

| Indicador | Definición | Meta |
|---|---|---|
| Tasa de finalización | % de colaboradores asignados que completan el módulo | ≥ 90% en 30 días |
| Tasa de aprobación al primer intento | % que aprueba sin reintentar | ≥ 75% |
| Calificación promedio | Promedio del examen entre todos los intentos | ≥ 85% |
| Tiempo promedio de finalización | Video + examen | ≤ 30 min (regla 3-8-30) |
| Requisitos legales evaluados a tiempo | % de requisitos legales con evaluación de cumplimiento realizada según la frecuencia definida | 100% |

---

**Código de documento:** PEM-ACAD-14001-M11-KIT-01
**Escuela:** Ambiental · Nivel 4 · Experto
**Cláusula normativa:** ISO 14001:2015 — 9.1.2
**Cumplimiento del Gold Standard:** sin desviaciones respecto a PEM-ACAD-GOLD-01
