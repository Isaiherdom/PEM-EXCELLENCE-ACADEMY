# PEM Excellence Academy — Kit de Módulo 10
### ISO 45001 · Cl. 8.1.3–8.1.4 · Gestión del Cambio y Contratación Externa · Nivel 4 Experto · Escuela de Seguridad y Salud

---

## 1. Prompt completo para Gamma (copiar y pegar)

```
Crea una presentación ejecutiva de máximo 8 diapositivas titulada "ISO 45001 — Módulo 10: Gestión del Cambio y Contratación Externa" para PEM Excellence Academy, Escuela de Seguridad de Production Enhancement México (PEM).

Mantén exactamente la misma identidad visual de los módulos anteriores: fondo oscuro casi negro, tipografía geométrica de alto contraste, acento ámbar único (#E8A33D), estilo minimalista ejecutivo, máximo 25 palabras por diapositiva.

Estructura (una idea por diapositiva):
1. Hook — "La cuadrilla entró a el sitio de trabajo sin inducción de seguridad. Nadie verificó su equipo de protección."
2. Qué es gestión del cambio en SST (Cl. 8.1.3) — evaluar antes de implementar.
3. Tipos de cambio: nuevos procesos, personal, requisitos legales, tecnología.
4. Qué es coordinación con contratistas (Cl. 8.1.4).
5. Ejemplo real: checklist de control antes de que un contratista ingrese a sitio.
6. Cambios no planificados también se revisan por sus consecuencias en seguridad.
7. Riesgo de un sitio de trabajo nueva sin roles de autoridad de SST ajustados.
8. Cierre — invitación a tomar el examen del Módulo 10 y obtener el certificado digital.

Usa imágenes de una inducción de seguridad a personal externo, una cuadrilla subcontratada con equipo de protección visible, y un sitio de trabajo nueva en construcción o arranque. Incluye "PEM Excellence Academy — Escuela de Seguridad — Módulo 10" en la portada.
```

---

## 2. Storyboard de las 8 diapositivas

| # | Diapositiva | Elemento visual dominante | Texto en pantalla |
|---|---|---|---|
| 1 | Hook | Cuadrilla entrando sin inducción | "Nadie verificó su equipo." |
| 2 | Gestión del cambio (8.1.3) | Icono de evaluación antes de flecha | "Evaluar antes de implementar" |
| 3 | Tipos de cambio | 4 iconos: proceso, personal, legal, tecnología | "No solo cambios grandes" |
| 4 | Coordinación con contratistas (8.1.4) | Personal externo con chaleco de PEM | "Su seguridad, también nuestra responsabilidad" |
| 5 | Checklist de contratistas | Lista de verificación (igual a la infografía) | "5 pasos antes de ingresar" |
| 6 | Cambio no planificado | Proveedor fallando, alternativa evaluada | "Lo inesperado también se revisa" |
| 7 | Riesgo | Sitio de trabajo nuevo sin roles claros | "Sin ajustar, el sistema empieza débil" |
| 8 | Cierre | CTA grande | "Toma el examen → obtén tu certificado" |

---

## 3. Imágenes recomendadas

- Inducción de seguridad siendo impartida a personal externo
- Cuadrilla subcontratada con equipo de protección visible en sitio de PEM
- Sitio de trabajo nuevo de PEM en construcción o arranque de operaciones
- Checklist o formato de control de contratistas, editable en Gamma
- Icono lineal de coordinación entre dos partes

Evitar: imágenes de "contratistas" genéricas de construcción sin relación con el sector energético.

---

## 4. Guion completo para avatar (3:30–4:30 min)

**[0:00–0:20 — Hook]**
"Una cuadrilla subcontratada entró a un sitio de trabajo de PEM a realizar mantenimiento. Nadie le dio inducción de seguridad, nadie verificó su equipo de protección. No fue mala intención — fue la ausencia de un proceso de coordinación con contratistas."

**[0:20–1:00 — Gestión del cambio]**
"La cláusula 8.1.3 pide que PEM evalúe cualquier cambio planificado que pueda afectar el desempeño de SST —antes de implementarlo—, ajustando controles, roles y recursos según sea necesario."

**[1:00–1:35 — Tipos de cambio]**
"Esto no se limita a cambios grandes. Incluye nuevos procesos o servicios, cambios en la fuerza laboral, cambios en requisitos legales, y cambios en el conocimiento o la tecnología que PEM utiliza."

**[1:35–2:15 — Coordinación con contratistas]**
"La cláusula 8.1.4 va sobre las compras y la contratación externa: PEM debe coordinar con contratistas para identificar los peligros y controlar los riesgos que su actividad introduce dentro de las instalaciones. La seguridad de un contratista dentro de PEM no es solo responsabilidad de su propia empresa."

**[2:15–2:55 — Checklist de contratistas]**
"En la práctica, esto significa cinco pasos antes de que cualquier contratista ingrese a sitio: inducción de seguridad, verificación de su equipo de protección, verificación de su competencia para la tarea, identificación de los peligros de su actividad, y un punto de contacto claro de PEM que sabe qué está haciendo y dónde."

**[2:55–3:30 — Cambio no planificado]**
"Y no todo cambio es decidido por PEM. Si un proveedor de equipo de protección falla repentinamente y hay que usar una alternativa, esa situación también se revisa por sus consecuencias en seguridad antes de que impacte a los trabajadores."

**[3:30–4:05 — Riesgo y cierre]**
"Si PEM incorpora un sitio de trabajo nueva sin ajustar los roles de autoridad de SST que vimos en los Módulos 2 y 3, ese sitio de trabajo puede operar sin que esté claro quién tiene autoridad para detener una operación insegura. Con esto terminas el Módulo 10. Toma el examen de 10 preguntas — necesitas 80% para aprobar — y genera tu certificado digital. Nos vemos en el Módulo 11."

*(Duración estimada: 4:10–4:30 min)*

---

## 5. Prompt para HeyGen (o Synthesia)

```
Avatar: el mismo personaje de los módulos anteriores de la Escuela de Seguridad, para continuidad de identidad.
Fondo: estudio neutro oscuro/gris grafito con acento ámbar (#E8A33D), idéntico a módulos anteriores.
Voz: español mexicano neutro, mismo ritmo pausado y directo.
Guion: usar el texto de la sección 4 de este documento, dividido en los mismos 6 bloques.
Duración objetivo: 4:10–4:30 minutos.
Gestos: énfasis en el bloque del checklist (contar los 5 pasos con la mano) y en el cierre.
Subtítulos: activados, español, sincronizados.
Salida: 1080p 16:9 para SharePoint/Teams + recorte 9:16 para celular.
```

---

## 6. Indicadores de desempeño del módulo

| Indicador | Definición | Meta |
|---|---|---|
| Tasa de finalización | % de colaboradores asignados que completan el módulo | ≥ 90% en 30 días |
| Tasa de aprobación al primer intento | % que aprueba sin reintentar | ≥ 75% |
| Calificación promedio | Promedio del examen entre todos los intentos | ≥ 85% |
| Tiempo promedio de finalización | Video + examen | ≤ 30 min (regla 3-8-30) |
| Contratistas con checklist completo | % de ingresos de personal externo con los 5 pasos del checklist verificados antes de iniciar actividad | 100% |

---

**Código de documento:** PEM-ACAD-45001-M10-KIT-01
**Escuela:** Seguridad y Salud · Nivel 4 · Experto
**Cláusula normativa:** ISO 45001:2018 — 8.1.3–8.1.4
**Cumplimiento del Gold Standard:** sin desviaciones respecto a PEM-ACAD-GOLD-01
