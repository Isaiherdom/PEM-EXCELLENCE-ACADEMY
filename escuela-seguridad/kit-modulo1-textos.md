# PEM Excellence Academy — Kit de Módulo 1
### ISO 45001 · Cl. 4 · Fundamentos y Contexto de SST · Nivel 1 Fundamentos · Escuela de Seguridad y Salud

Primer módulo de la Escuela de Seguridad y Salud. Replica exactamente la estructura del Gold Standard (PEM-ACAD-GOLD-01); acento visual ámbar (#E8A33D), reservado para esta escuela desde la Guía de Identidad Visual.

---

## 1. Prompt completo para Gamma (copiar y pegar)

```
Crea una presentación ejecutiva de máximo 8 diapositivas titulada "ISO 45001 — Módulo 1: Fundamentos y Contexto de SST" para PEM Excellence Academy, la Escuela de Seguridad y Salud de la universidad corporativa de Production Enhancement México (PEM), empresa de servicios petroleros especializada en compresión reciprocante, well testing y generación de energía.

Estilo visual: minimalista, ejecutivo, premium, inspirado en Apple Keynote, Tesla y Microsoft. Fondo oscuro casi negro (#0B0E0C), tipografía geométrica de alto contraste, un solo color de acento ámbar (#E8A33D), muy poco texto por diapositiva (máximo 25 palabras).

Estructura (una idea por diapositiva):
1. Hook — "Un mecánico estuvo a punto de lesionarse cerca de un componente en movimiento. No hubo lesión. Nunca se reportó."
2. Qué es el contexto de SST (Cl. 4.1) — cuestiones internas y externas que afectan la prevención de lesiones.
3. El trabajador como parte interesada central — consulta y participación, no solo protección pasiva.
4. Alcance del sistema de SST — actividades, lugares y personas, incluidos contratistas.
5. ISO 45001 en una frase — anticipar el peligro, no reaccionar después del accidente.
6. Ejemplo real: componentes en movimiento y presión en compresores.
7. Riesgo de un contexto de SST mal identificado — inconsistencia entre sitios de trabajo.
8. Cierre — invitación a tomar el examen del Módulo 1 y obtener el certificado digital.

Usa imágenes reales de operación petrolera con equipo de protección visible, un mecánico trabajando cerca de un compresor con protocolo de seguridad, y un mapa simple de partes interesadas centrado en el trabajador. Incluye "PEM Excellence Academy — Escuela de Seguridad y Salud — Módulo 1" en la portada.
```

---

## 2. Storyboard de las 8 diapositivas

| # | Diapositiva | Elemento visual dominante | Texto en pantalla |
|---|---|---|---|
| 1 | Hook | Componente en movimiento, silueta cercana | "Nunca se reportó." |
| 2 | Contexto de SST | Icono de PEM rodeado de factores | "Lo que puede lastimar, se anticipa" |
| 3 | Trabajador al centro | Mapa centrado en el trabajador (igual a la infografía) | "Su voz, no solo su protección" |
| 4 | Alcance del sistema | Iconos de actividades, lugares, contratistas | "Todos los que pueden verse afectados" |
| 5 | ISO 45001 en una frase | Fondo ámbar, una sola línea grande | "Anticipar, no reaccionar" |
| 6 | Ejemplo real | compresor con componente en movimiento resaltado | "Presión, partes móviles, campo" |
| 7 | Riesgo | sitios de trabajo con entendimiento distinto | "Sin contexto claro, inconsistencia" |
| 8 | Cierre | CTA grande | "Toma el examen → obtén tu certificado" |

---

## 3. Imágenes recomendadas

- Mecánico o técnico trabajando cerca de un compresor con protocolo de seguridad visible
- Equipo de protección personal (EPP) correctamente utilizado en campo
- Mapa o diagrama de nodos centrado en el trabajador, editable en Gamma
- Componente en movimiento de un compresor, con señalización de seguridad
- Icono lineal de consulta/participación (dos personas dialogando)

Evitar: imágenes de accidentes reales o dramatizados de forma alarmista.

---

## 4. Guion completo para avatar (3:30–4:30 min)

**[0:00–0:20 — Hook]**
"Un mecánico estuvo a punto de sufrir una lesión seria trabajando cerca de un componente en movimiento de un compresor. No hubo lesión — pero ese casi-incidente nunca se reportó. La señal estaba ahí. Lo que faltó fue entender el contexto real de riesgo de la operación."

**[0:20–1:00 — Contexto de SST]**
"La cláusula 4.1 de ISO 45001 pide que PEM identifique las cuestiones internas y externas que afectan su capacidad de prevenir lesiones y enfermedades relacionadas con el trabajo: el tipo de operación, las condiciones de campo, y la regulación laboral vigente."

**[1:00–1:40 — El trabajador al centro]**
"Aquí hay algo que distingue a esta norma de las dos anteriores que ya viste en la Academia: ISO 45001 exige explícitamente la consulta y participación de los propios trabajadores. No son solo destinatarios pasivos de las decisiones de seguridad — su voz es parte formal del sistema."

**[1:40–2:15 — Alcance del sistema]**
"El alcance del sistema de SST de PEM tiene que cubrir todas las actividades, lugares de trabajo y personas que pueden verse afectadas por los riesgos de la operación — incluido el personal de contratistas cuando trabaja en las instalaciones de PEM."

**[2:15–2:50 — ISO 45001 en una frase]**
"Si tuviéramos que resumir esta norma en una idea, sería: anticipar los peligros de la operación antes de que causen una lesión, en lugar de reaccionar después de un accidente. El mismo pensamiento basado en riesgos que ya conoces, aplicado ahora a la seguridad de las personas."

**[2:50–3:25 — Ejemplo real]**
"Los compresores reciprocantes, por su naturaleza, involucran componentes en movimiento y presión — condiciones que exigen control activo, no indiferencia, para prevenir una lesión."

**[3:25–4:00 — Riesgo y cierre]**
"Cuando dos sitios de trabajo entienden de forma distinta qué cubre el sistema de SST, se genera el mismo tipo de inconsistencia que ya viste en las escuelas de Calidad y Ambiental. Con esto terminas el Módulo 1 de la Escuela de Seguridad y Salud. Toma el examen de 10 preguntas — necesitas 80% para aprobar — y genera tu certificado digital. Nos vemos en el Módulo 2."

*(Duración estimada: 4:00–4:20 min)*

---

## 5. Prompt para HeyGen (o Synthesia)

```
Avatar: nuevo personaje para la Escuela de Seguridad y Salud (a validar si se reutiliza un avatar existente o se define uno propio — ver Gold Standard sección 1.2), profesional mexicano, 35-45 años, business casual, tono cercano con autoridad técnica.
Fondo: estudio neutro oscuro/gris grafito con acento ámbar (#E8A33D), coherente con el color reservado a esta escuela.
Voz: español mexicano neutro, ritmo pausado y directo.
Guion: usar el texto de la sección 4 de este documento, dividido en los mismos 6 bloques.
Duración objetivo: 4:00–4:20 minutos.
Gestos: manos visibles, énfasis en el bloque del trabajador al centro (gesto de señalarse a sí mismo) y en el cierre.
Subtítulos: activados, español, sincronizados.
Salida: 1080p 16:9 para SharePoint/Teams + recorte 9:16 para celular.
```

---

## 6. Indicadores de desempeño del módulo

| Indicador | Definición | Meta |
|---|---|---|
| Tasa de finalización | % de colaboradores asignados que completan el módulo | ≥ 90% en 30 días |
| Tasa de aprobación al primer intento | % que aprueba sin reintentar | ≥ 75% |
| Calificación promedio | Promedio del examen entre todos los intentos | ≥ 85% |
| Tiempo promedio de finalización | Video + examen | ≤ 30 min (regla 3-8-30) |
| Casi-incidentes reportados a tiempo | % de casi-incidentes reportados dentro de las primeras 24 horas tras completar el módulo | Monitoreo — tendencia creciente esperada |

---

**Código de documento:** PEM-ACAD-45001-M1-KIT-01
**Escuela:** Seguridad y Salud · Nivel 1 · Fundamentos
**Cláusula normativa:** ISO 45001:2018 — 4
**Cumplimiento del Gold Standard:** sin desviaciones respecto a PEM-ACAD-GOLD-01
