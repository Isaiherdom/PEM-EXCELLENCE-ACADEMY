# PEM Excellence Academy — Kit de Módulo 8
### ISO 45001 · Cl. 7.4 · Comunicación y Consulta · Nivel 3 Avanzado · Escuela de Seguridad y Salud

---

## 1. Prompt completo para Gamma (copiar y pegar)

```
Crea una presentación ejecutiva de máximo 8 diapositivas titulada "ISO 45001 — Módulo 8: Comunicación y Consulta" para PEM Excellence Academy, Escuela de Seguridad de Production Enhancement México (PEM).

Mantén exactamente la misma identidad visual de los módulos anteriores: fondo oscuro casi negro, tipografía geométrica de alto contraste, acento ámbar único (#E8A33D), estilo minimalista ejecutivo, máximo 25 palabras por diapositiva.

Estructura (una idea por diapositiva):
1. Hook — "El accidente debía notificarse en el plazo legal. Nadie lo hizo a tiempo."
2. Qué exige Cl. 7.4 — qué, cuándo, con quién, cómo y quién comunica.
3. Ejemplo real: la matriz de notificación de un accidente a la autoridad laboral.
4. Comunicación interna — un peligro detectado en un sitio de trabajo compartido con las otras 4.
5. La plática de seguridad (toolbox talk) como comunicación efectiva diaria.
6. Consulta — doble vía, no solo informar, conectado a los Módulos 2 y 3.
7. Riesgo: un procedimiento de comunicación que existe pero no se ejecuta con rapidez.
8. Cierre — invitación a tomar el examen del Módulo 8 y obtener el certificado digital.

Usa imágenes de una plática de seguridad real antes del turno, una notificación regulatoria siendo enviada, y un comité mixto en sesión. Incluye "PEM Excellence Academy — Escuela de Seguridad — Módulo 8" en la portada.
```

---

## 2. Storyboard de las 8 diapositivas

| # | Diapositiva | Elemento visual dominante | Texto en pantalla |
|---|---|---|---|
| 1 | Hook | Reloj corriendo sobre un reporte de accidente | "El plazo se venció." |
| 2 | Comunicación (7.4) | 5 iconos: qué, cuándo, quién, cómo, quién comunica | "5 preguntas antes de notificar" |
| 3 | Ejemplo real | Matriz de comunicación (igual a la infografía) | "El plazo legal, no el propio" |
| 4 | Comunicación interna | Sitios de trabajo compartiendo un peligro detectado | "Un peligro, sitios de trabajo" |
| 5 | Toolbox talk | Plática de seguridad real antes del turno | "Información justo a tiempo" |
| 6 | Consulta | Trabajador y supervisor dialogando, no solo informando | "Doble vía, no solo aviso" |
| 7 | Riesgo | Procedimiento de comunicación lento en la práctica | "Existir no es suficiente" |
| 8 | Cierre | CTA grande | "Toma el examen → obtén tu certificado" |

---

## 3. Imágenes recomendadas

- Plática de seguridad ("toolbox talk") real antes de iniciar el turno
- Notificación o reporte regulatorio siendo enviado (composición abstracta)
- Comité mixto de seguridad en sesión
- Icono lineal de las 5 preguntas de comunicación
- Bitácora o registro de incidente de SST

Evitar: iconografía genérica de trámite gubernamental sin relación con el sector energético.

---

## 4. Guion completo para avatar (3:30–4:30 min)

**[0:00–0:20 — Hook]**
"Ocurrió un accidente de trabajo en un sitio de trabajo de PEM que, por ley, debía notificarse a la autoridad laboral dentro de un plazo determinado. Nadie lo notificó a tiempo — no por ocultarlo, sino porque no estaba claro quién debía hacerlo ni cuándo."

**[0:20–1:00 — Comunicación]**
"La cláusula 7.4 de ISO 45001 pide que PEM determine, de forma deliberada, qué comunicar sobre el sistema de SST, cuándo, con quién, cómo, y quién es el responsable — tanto dentro de la empresa como hacia afuera, incluyendo obligaciones legales de notificación."

**[1:00–1:40 — Ejemplo real]**
"Así se ve bien hecho: ante un accidente que cumple los criterios legales, la matriz de comunicación deja claro qué se notifica, cuándo —dentro del plazo legal—, a quién —autoridad laboral y comité mixto—, cómo —notificación formal más registro interno—, y quién lo hace."

**[1:40–2:15 — Comunicación interna]**
"La misma disciplina aplica hacia adentro: un peligro detectado en un sitio de trabajo —como un punto ciego de montacargas— debe compartirse con las otras cuatro, porque el mismo riesgo puede presentarse ahí también."

**[2:15–2:50 — Toolbox talk]**
"Una plática de seguridad antes de iniciar el turno es comunicación efectiva en su forma más simple: información relevante llegando a quien la necesita, justo antes de que la necesite."

**[2:50–3:25 — Consulta]**
"El título de esta cláusula no es solo 'comunicación' — es 'comunicación y consulta'. Consultar es un proceso de doble vía: PEM no solo informa, busca y considera la opinión de los trabajadores antes de decisiones que afectan su seguridad, como vimos en los Módulos 2 y 3."

**[3:25–4:00 — Riesgo y cierre]**
"Un procedimiento de comunicación que existe en documento, pero que en la práctica tarda semanas en circular, no cumple su función real. Con esto terminas el Módulo 8. Toma el examen de 10 preguntas — necesitas 80% para aprobar — y genera tu certificado digital. Nos vemos en el Módulo 9."

*(Duración estimada: 4:00–4:20 min)*

---

## 5. Prompt para HeyGen (o Synthesia)

```
Avatar: el mismo personaje de los módulos anteriores de la Escuela de Seguridad, para continuidad de identidad.
Fondo: estudio neutro oscuro/gris grafito con acento ámbar (#E8A33D), idéntico a módulos anteriores.
Voz: español mexicano neutro, mismo ritmo pausado y directo.
Guion: usar el texto de la sección 4 de este documento, dividido en los mismos 6 bloques.
Duración objetivo: 4:00–4:20 minutos.
Gestos: énfasis en el bloque de la matriz de comunicación (contar los 5 elementos con la mano) y en el cierre.
Subtítulos: activados, español, sincronizados.
Salida: 1080p 16:9 para SharePoint/Teams + recorte 9:16 para celular.
```

---

## 6. Indicadores de desempeño del módulo

| Indicador | Definición | Meta |
|---|---|---|
| Tasa de finalización | % de colaboradores asignados que completan el módulo | ≥ 90% en 30 días |
| Tasa de aprobación al primer intento | % que aprueba sin reintentar | ≥ 75% |
| Calificación promedio | Promedio del examen entre todos los intentos | ≥ 85% |
| Tiempo promedio de finalización | Video + examen | ≤ 30 min (regla 3-8-30) |
| Notificaciones de accidentes dentro de plazo | % de eventos que requirieron notificación legal, reportados dentro del plazo | 100% |
| Pláticas de seguridad realizadas | % de turnos con toolbox talk documentado antes de iniciar operación | ≥ 95% |

---

**Código de documento:** PEM-ACAD-45001-M8-KIT-01
**Escuela:** Seguridad y Salud · Nivel 3 · Avanzado
**Cláusula normativa:** ISO 45001:2018 — 7.4
**Cumplimiento del Gold Standard:** sin desviaciones respecto a PEM-ACAD-GOLD-01
