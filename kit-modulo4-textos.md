# PEM Excellence Academy — Kit de Módulo 4
### ISO 9001 · Cl. 6.1 · Riesgos y Oportunidades · Nivel 2 Intermedio · Escuela de Calidad

---

## 1. Prompt completo para Gamma (copiar y pegar)

```
Crea una presentación ejecutiva de máximo 8 diapositivas titulada "ISO 9001 — Módulo 4: Riesgos y Oportunidades" para PEM Excellence Academy, la universidad corporativa de Production Enhancement México (PEM).

Mantén exactamente la misma identidad visual de los Módulos 1, 2 y 3: fondo oscuro casi negro, tipografía geométrica de alto contraste, acento verde único (#16C270), estilo minimalista ejecutivo inspirado en Apple, Tesla y Microsoft, máximo 25 palabras por diapositiva.

Estructura (una idea por diapositiva):
1. Hook — "Ese compresor no falló de repente. Llevaba tres semanas dando señales que nadie reportó."
2. Qué es pensamiento basado en riesgos en PEM — anticipar, no reaccionar.
3. Riesgo vs. oportunidad — Cl. 6.1 exige determinar ambos, no solo evitar lo malo.
4. Ejemplo de riesgo — vibración anormal detectada a tiempo vs. ignorada.
5. Ejemplo de oportunidad — evaluar un nuevo servicio o cliente de forma planificada.
6. Las acciones deben ser proporcionales — ni exageradas ni insuficientes, integradas al proceso y con eficacia evaluada.
7. Tu rol — las señales tempranas se detectan primero en campo, no en oficina.
8. Cierre — invitación a tomar el examen del Módulo 4 y obtener el certificado digital.

Usa imágenes de un sensor o indicador de vibración industrial, un técnico inspeccionando un componente, y una gráfica simple de línea de tiempo (señal temprana → acción → control). Incluye "PEM Excellence Academy — Módulo 4" en la portada.
```

---

## 2. Storyboard de las 8 diapositivas

| # | Diapositiva | Elemento visual dominante | Texto en pantalla |
|---|---|---|---|
| 1 | Hook | compresor con línea de tiempo de fondo mostrando semanas | "Llevaba semanas dando señales." |
| 2 | Qué es pensamiento basado en riesgos | Icono: lupa sobre una señal temprana | "Anticipar, no reaccionar" |
| 3 | Riesgo vs. oportunidad | Dos columnas simples, una roja/ámbar, una verde | "Dos caras del mismo requisito" |
| 4 | Ejemplo de riesgo | Sensor de vibración con dato resaltado | "La señal que nadie reportó" |
| 5 | Ejemplo de oportunidad | Mapa simple con un nuevo nodo de cliente potencial | "Evaluar antes de aceptar" |
| 6 | Acciones proporcionales | Balanza simple: riesgo vs. esfuerzo de la acción | "Ni de más, ni de menos" |
| 7 | Tu rol | Operador señalando un indicador en campo | "Tú ves la señal primero" |
| 8 | Cierre | CTA grande | "Toma el examen → obtén tu certificado" |

---

## 3. Imágenes recomendadas

- Sensor o pantalla de monitoreo de vibración/temperatura en equipo industrial
- Técnico inspeccionando un componente crítico en campo
- Gráfica de línea de tiempo simple (estilo de la infografía del módulo), editable en Gamma
- Balanza o ícono de equilibrio para representar "acción proporcional"
- Mapa simple con nodo nuevo para representar oportunidad de negocio

Evitar: iconografía genérica de "gestión de riesgos" tipo semáforo corporativo sin conexión a campo real.

---

## 4. Guion completo para avatar (3:30–4:30 min)

**[0:00–0:20 — Hook]**
"Ese compresor no falló de la nada. Cuando se revisó el historial después de la falla, llevaba tres semanas mostrando vibración anormal — y nadie lo reportó como riesgo. La señal estaba ahí. Lo que faltó fue actuar sobre ella a tiempo."

**[0:20–1:00 — Qué es pensamiento basado en riesgos]**
"La cláusula 6.1 de ISO 9001 le pide a PEM determinar los riesgos y oportunidades que hay que abordar para que el sistema logre sus resultados. No es un ejercicio de oficina una vez al año — es la disciplina de anticipar antes de que una señal se convierta en una falla real, o de aprovechar antes de que una oportunidad se pierda."

**[1:00–1:40 — Riesgo vs. oportunidad]**
"Y aquí hay algo que se pasa por alto: la norma no solo habla de evitar lo malo. Habla igual de riesgos que de oportunidades. Un riesgo es un efecto no deseado que hay que prevenir o reducir. Una oportunidad es una posibilidad de mejora — un nuevo servicio, un nuevo cliente — que también se evalúa de forma planificada, no se acepta por impulso comercial."

**[1:40–2:25 — Ejemplo de riesgo]**
"Volvamos al compresor. La vibración anormal era una señal temprana de riesgo. Si se hubiera reportado la primera semana, se habría podido programar una revisión preventiva — una acción proporcional al riesgo real. En cambio, al ignorarla, la señal escaló hasta convertirse en un paro no planeado y un costo de reparación mayor."

**[2:25–3:00 — Ejemplo de oportunidad]**
"Del otro lado, cuando PEM evalúa ofrecer un nuevo servicio a un cliente distinto de PEMEX o CFE, esa oportunidad también se analiza con el mismo cuidado: ¿qué capacidad tiene PEM para cumplirlo con la misma calidad? Evaluar la oportunidad antes de comprometerse es tan parte de la Cl. 6.1 como prevenir un riesgo."

**[3:00–3:40 — Acciones proporcionales]**
"La norma pide que las acciones frente a un riesgo u oportunidad sean proporcionales a su impacto, que se integren al proceso normal —no como un esfuerzo aislado— y que se evalúe si realmente funcionaron. Eso cierra el mismo ciclo PHVA del Módulo 1: planificar la acción, ejecutarla, verificar si el riesgo se controló, y ajustar si no fue así."

**[3:40–4:15 — Tu rol y cierre]**
"Las señales tempranas —una vibración, un ruido distinto, un retraso de un proveedor— se detectan primero en campo, no en un escritorio de oficina. Reportarlas a tiempo es la parte más importante que tú tienes en este requisito. Con esto terminas el Módulo 4. Toma el examen de 10 preguntas — necesitas 80% para aprobar — y genera tu certificado digital. Nos vemos en el Módulo 5."

*(Duración estimada: 4:10–4:30 min)*

---

## 5. Prompt para HeyGen (o Synthesia)

```
Avatar: el mismo personaje de los Módulos 1, 2 y 3, para mantener continuidad de identidad en la Escuela de Calidad.
Fondo: estudio neutro oscuro/gris grafito con acento verde (#16C270), idéntico a módulos anteriores.
Voz: español mexicano neutro, mismo ritmo pausado y directo.
Guion: usar el texto de la sección 4 de este documento, dividido en los mismos 6 bloques.
Duración objetivo: 4:10–4:30 minutos.
Gestos: énfasis en el bloque "riesgo vs. oportunidad" (gesto de balanza con las manos) y en el cierre.
Subtítulos: activados, español, sincronizados.
Salida: 1080p 16:9 para SharePoint/Teams + recorte 9:16 para celular.
```

---

## 6. Indicadores de desempeño del módulo

| Indicador | Definición | Meta |
|---|---|---|
| Tasa de finalización | % de colaboradores asignados que completan el módulo | ≥ 90% en 30 días |
| Tasa de aprobación al primer intento | % que aprueba sin reintentar | ≥ 75% |
| Calificación promedio | Promedio del examen entre todos los intentos | ≥ 85% |
| Tiempo promedio de finalización | Video + examen | ≤ 30 min (regla 3-8-30) |
| Señales tempranas reportadas | Número de riesgos u oportunidades reportados por colaboradores tras completar el módulo (indicador cualitativo de aplicación real) | Monitoreo — tendencia creciente esperada |

---

**Código de documento:** PEM-ACAD-M4-KIT-01
**Escuela:** Calidad · Nivel 2 · Intermedio
**Cláusula normativa:** ISO 9001:2015 — 6.1
**Cumplimiento del Gold Standard:** sin desviaciones respecto a PEM-ACAD-GOLD-01
